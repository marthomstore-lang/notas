...
async function main() {
    try {
        console.log("Checking all levels matching '2° Básico' or similar:");
        const levels = await db.all("SELECT id, name FROM levels WHERE name ILIKE '%2°%básico%' OR name ILIKE '%2%basico%'");
        console.log(levels);
