...
async function main() {
    try {
        console.log("Checking all levels matching '1° Básico' or similar:");
        const levels = await db.all("SELECT id, name FROM levels WHERE name ILIKE '%1°%básico%' OR name ILIKE '%1%basico%'");
        console.log(levels);
