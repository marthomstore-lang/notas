...
exports.registerEnrollment = void 0;
...
const registerEnrollment = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const levelRes = yield client.query('SELECT * FROM levels WHERE id = ?', [levelId]);
        ...
        if (guardiansData && guardiansData.length > 0) {
            for (const g of guardiansData) {
                yield client.query(`
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                `, [(0, uuid_1.v4)(), studentId, g.guardian_type, g.run, g.full_name, g.relationship, g.phone, g.email, g.address]);
            }
        }
        if (healthData) {
            yield client.query(`
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            `, [(0, uuid_1.v4)(), studentId, healthData.blood_type, healthData.allergies, healthData.chronic_diseases, healthData.general_observations]);
        }
        // Matrícula
        yield client.query(`
            INSERT INTO enrollments (id, student_id, level_id, academic_year)
            VALUES (?, ?, ?, ?)
        `, [(0, uuid_1.v4)(), studentId, levelId, academicYear || 2026]);
        // Actualizar cupo
        yield client.query('UPDATE levels SET current_enrolled = current_enrolled + 1 WHERE id = ?', [levelId]);
